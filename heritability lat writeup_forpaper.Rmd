...
# NB This option will not work for external users, as it requires access to files with confidential data.
#---------------------------------------------------------------
```
...
```{r computecorrs}
  ...
  oldnewLIcor <-
  cor(doubledata$peakLI, doubledata$lat_index, use = 'pairwise.complete.obs')
  oldnewLIcorDF <- sum(is.na(doubledata$peakLI)) - 2
  
  #check split half reliablity
  splithalfr <-
  cor(doubledata$my_li_mean_even, doubledata$my_li_mean_odd, use = 'pairwise.complete.obs')
  #--------------------------------------------------------------------------------------
```
...
dataforOSF<-dplyr::select(doubledata,c(rand_id,fam_randid,age_at_test,female,zygosity,n_language_low,myinclude,twin,n_trials,lat_index,myse_mean,qhp_handedness,ehp_handedness,lang_probs,li_cat,ehp_write,peakLI,peak_latcat,mean_left_flow,mean_right_flow,Lflow.resid,Rflow.resid,my_li_mean_odd,my_li_mean_even,mean_words,doppexcl,female2,n_language_low2,myinclude2,twin2,n_trials2,lat_index2,myse_mean2,qhp_handedness2,ehp_handedness2,lang_probs2,li_cat2,ehp_write2,peakLI2,peak_latcat2,mean_left_flow2,mean_right_flow2,Lflow.resid2,Rflow.resid2,doppexcl2,MZDZ,langconcord,doppout,zygosex))
