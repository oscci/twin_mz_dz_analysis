...
summary(m1) #confirms that all C paths are either nonsignificant or negative!
...
```{r batesmultiflow,echo=FALSE, include=FALSE} 
#Multivariate with both flow measures - just curious
#Not included in paper: 
#Impression is you can get reasonable fit with either AE or CE, but you can't drop both
multivar<-1
if(multivar==1){
  umx_set_auto_plot(TRUE)
  #reformat data to fit umx format
mytemp<-dplyr::select(doubledata2,mean_left_flow,mean_right_flow,mean_left_flow2,mean_right_flow2,MZDZ)
colnames(mytemp)[1:4]<-c('Lflow_1','Rflow_1','Lflow_2','Rflow_2')
mzData<-filter(mytemp,MZDZ==1)
dzData<-filter(mytemp,MZDZ==2)

m1 = umxACE(selDVs = c("Lflow","Rflow"), sep="_", mzData = mzData, dzData= dzData)
summary(m1) #confirms that all C paths are either nonsignificant or negative!
...
#now drop A terms from m1
 m4 = umxModify(m1, regex = "a_r.c[1-2]", name="dropA_12", comparison= TRUE)
 umxSummary(m4) #displays path diagram 
umxCompare(m1, m4)
mxCompare(m1,m4) #p = .107, i.e. can drop A terms as well without sig loss of fit!
}
```
