...
library(officer)
library(umx)
library(RVAideMemoire) #for bootstrapped CIs on Spearman correlations
...
data.short<-dplyr::select(mydata,record_id,fam_id,age_at_test,female,zygosity,n_language_low,myinclude,twin,n_trials, lat_index,qhp_handedness,ehp_handedness,lang_probs,li_cat,ehp_handedness,qhp_handedness,ehp_write,peakLI,peakLIse,peak_latcat,mean_left_flow,mean_right_flow,mean_words,myse_mean)
...
## Abstract
**Background** 
It is unclear whether individual differences in language lateralisation are heritable. Prior studies show  heritability around 0.25 for the related trait of handedness, and studies of structural brain asymmetry  obtained estimates in a similar or lower range. This report describes heritability estimates using functional language laterality and handedness phenotypes in a twin sample previously reported by Wilson and Bishop (2018).
...
Two recent studies by the ENIGMA consortium looked at brain asymmetry for subcortical and cortical structures respectively (Guadalupe et al., 2017; Kong et al., 2018). The first study included analyses of 1170 individuals from 71 genetically informative pedigrees.  Subcortical asymmetries, though small in magnitude, were significantly heritable for four of the seven regions, with genetic factors accounting for between .15 and .27 of variation (Guadalupe et al., 2017).  Cortical brain asymmetries were more substantial, and in an increased pedigree sample, significant heritability was found for six of 34 measures of cortical thickness and four measures of area, after Bonferroni correction. These analyses were repeated in another genetically informative sample that included 143 MZ pairs and 85 DZ pairs; significant heritability for one of the thickness measures (parahippocampal gyrus) was replicated in this new sample, as was one of the area measures (superior temporal), with heritability estimates ranging from .15 to .23.  

It is unclear how these structural asymmetries relate to functional language laterality; there was no evidence of any relationship with handedness in very large meta-analyses done by Guadalupe et al (2017) or Kong et al (2017). 
...
Following a suggestion by Francks (2019), we also conducted genetic analysis of the left- and right-sided blood flow measures. 
...
#Subsequent analysis shows flow measures appear to have shared environment effect 
#ie correlations significant regardless of zygosity.
#Could this be an age or sex effect?
#Try with scores residualised on these, and also on mean words spoken
 #In fact, this makes v little difference to the flow measures
...
doubledata$Lflow.resid<-doubledata$mean_left_flow -(mylm$coefficients[1,1]+
                                        ...
                                        mylm$coefficients[3,1]*doubledata$female+
                                          mylm$coefficients[4,1]*doubledata$mean_words)
...
mycols<-c("ehp_handedness", "qhp_handedness", "lat_index","peakLI","peakcatbinary","Lflow.resid","Rflow.resid")
...
colnames(uutable)<-c('Measure','N MZ pair','N DZ pair','rMZ','rDZ','a2','95% CI','chisq','p','RMSEA')
...
uutable$rMZ[thisrow] <- paste0(round(sp.m$estimate,3)," (",round(sp.m$conf.int[1],3),' to ',round(sp.m$conf.int[2],3),')')
sp.d<-spearman.ci(dzData$X_T1, dzData$X_T2, nrep = 1000, conf.level = 0.95)
uutable$rDZ[thisrow] <- paste0(round(sp.d$estimate,3)," (",round(sp.d$conf.int[1],3),' to ',round(sp.d$conf.int[2],3),')')
...
#add bootstrapped CI computed with bootstrap_ci.Rmd
  ...
  #########################################################################
  #need to specify directory where bootstrapped estimates are stored here!
  setwd("/Users/dorothybishop/twin_mz_dz_analysis/bootstrap_estimates")
  #########################################################################
  
bootfile<-paste0(mycols[j],'_bootstraps_umx_5000.csv')
...
myCI<-(paste(round(q3,3),'-',round(q4,3)))
uutable$`95% CI`[j]<-myCI
}
}
...
dataforOSF<-dplyr::select(doubledata,c(rand_id,fam_randid,age_at_test,female,zygosity,n_language_low,myinclude,twin,n_trials,lat_index,myse_mean,qhp_handedness,ehp_handedness,lang_probs,li_cat,ehp_write,peakLI,peak_latcat,mean_left_flow,mean_right_flow,mean_words,doppexcl,female2,n_language_low2,myinclude2,twin2,n_trials2,lat_index2,myse_mean2,qhp_handedness2,ehp_handedness2,lang_probs2,li_cat2,ehp_write2,peakLI2,peak_latcat2,mean_left_flow2,mean_right_flow2,doppexcl2,MZDZ,langconcord,doppout,zygosex))
